...
/* 	function newcustomer(){
	...
	function getCustomer(curPage1) {
		...
		customerName=document.getElementById("text1").value;
		type=document.getElementById("text2").value;
		groupCompany=document.getElementById("text3").value;
		customerCode=document.getElementById("text4").value;
		status=document.getElementById("text5").value;
		corporation=document.getElementById("text6").value;	
		...
		$.getJSON(urlGet,function(data) {
				...
				$.each(data,function(i, entry) {//遍历JSON
						row += "<tr><td>"
								...
								+ "</td><td>"
								+ entry.Sta
								+ "</td><td><a href='CustomerDetailed.action?country="+
										entry.Coun+"&customerName="+entry.CN+"&businessManager="+entry.BM+"'><button>查看</button></a><button>编辑</button>"
								+"</td><td><a href='ConfirmAction.action?status="+entry.Sta+"'><button>确认</button>"
								+"</td></tr>"
					});
				$("#table1").html(row);
